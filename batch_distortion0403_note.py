...
folder="/mnt/hwfile/opendatalab/bigdata_mineru/liuzheng/Mineru/dataset/Final/Document/notes-1200k"
file_dict={
    "mineru2_notes-math_50k.json": 2, # 每隔2取1个
    "mineru2_notes-pdf_60k.json": 2,
    "mineru2_notes-line_90k.json": 2,
    "mineru2_notes-synth-zh_400k.json": 8,
    "mineru2_notes-synth-mixed_400k.json": 8,
}
...
if __name__ == '__main__':
    ...
    for meta_data_path in file_dict.keys():
        ...
        for idx in tqdm(range(len(my_dataset))):
            try:
                ...
                image_name = image_name.split("s3://doc-parse-huawei/")[-1]
    ...
    for data in all_meta_data:
        image_name = data["image"].split("s3://doc-parse-huawei/")[-1]
    ...
    target_meta_data_path = os.path.join(target_folder, f"mineru2_notes_{args.distortion}_{args.chunk_idx:02d}_{args.chunk_num:02d}.json")
