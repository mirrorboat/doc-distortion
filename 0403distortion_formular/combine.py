...
for distortion in distortion_ls:
    ...
    for chunk_idx in tqdm(range(chunk_num)):
        path=f"mineru2_formular_{distortion}_{chunk_idx:02d}_{chunk_num:02d}_fixlabel.json"
    ...
    # 将合并后的数据保存到文件
    combined_path=f"mineru2_formular_{distortion}_combined_25k_fixlabel.json"
